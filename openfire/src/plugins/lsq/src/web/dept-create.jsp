...
<%  // Get parameters //
    ...
    if (cancel) {
        if (groupName == null) {
            ...
            response.sendRedirect("dept-edit.jsp?group=" + URLEncoder.encode(groupName, "UTF-8"));    
        }
        return;
    }
    ...
    if (create) {
        ...
        if (errors.size() == 0) {
            try {
                ...
                response.sendRedirect("dept-edit.jsp?creategroupsuccess=true&group=" + URLEncoder.encode(newGroup.getName(), "UTF-8"));
                return;
            }
    ...
    if (edit) {
        ...
        if (errors.size() == 0) {
            try {
                ...
                // Successful, so redirect
                response.sendRedirect("dept-edit.jsp?groupChanged=true&group=" + URLEncoder.encode(group.getName(), "UTF-8"));
                return;
            }
            catch (Exception e) {
                errors.put("general", "");
...
<% if (groupName == null) { %>
<meta name="pageID" content="dept-create"/>
<% }
   else { %>
...
<%  if (errors.get("general") != null) { %>
    ...
    <tbody>
        <tr>
            <td class="jive-icon">
                <img src="<%=basePath%>images/error-16x16.gif" width="16" height="16" border="0" alt="">
            </td>
...
<form name="f" action="dept-create.jsp" method="post">
