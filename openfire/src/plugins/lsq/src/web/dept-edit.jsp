<%@ page import="org.jivesoftware.openfire.PresenceManager,
                 ...
                 org.jivesoftware.openfire.user.UserNotFoundException" contentType="text/html; charset=utf-8"
%>
...
<%
    ...
    String [] deptUsers = ParamUtils.getParameters(request, "deptUsers");
    ...
    if (cancel) {
        response.sendRedirect("dept-summary.jsp");
        return;
    }
    ...
    if (update) {
        ...
        if (errors.isEmpty()) {
            ...
            response.sendRedirect("dept-edit.jsp?group=" + URLEncoder.encode(groupName, "UTF-8") + "&groupChanged=true");
            return;
        }
        else {
    ...
    if (updateMember) {
        ...
        // Get admin list and compare it the admin posted list.
        response.sendRedirect("dept-edit.jsp?group=" + URLEncoder.encode(groupName, "UTF-8") + "&updatesuccess=true");
        return;
    }
        ...
        if (count > 0) {
            response.sendRedirect("dept-edit.jsp?group=" +
    ...
    }else if(add && deptUsers != null){
   		int count = 0;
    	for(String selectUserName:deptUsers){
    		if(null==selectUserName||"".equals(selectUserName)){
    			continue;
    		}
    		selectUserName = selectUserName.trim();
    		selectUserName = selectUserName.toLowerCase();
            ...
            try {
                ...
                if (selectUserName.indexOf('@') == -1) {
                    // No @ was found so assume this is a JID of a local user
                    selectUserName = JID.escapeNode(selectUserName);
                    selectUserName = Stringprep.nodeprep(selectUserName);
                    UserManager.getInstance().getUser(selectUserName);
                    added = group.getMembers().add(webManager.getXMPPServer().createJID(selectUserName, null));
                }
                else {
                    // Admin entered a JID. Add the JID directly to the list of group members
                    added = group.getMembers().add(new JID(selectUserName));
                    if (!SecurityAuditManager.getSecurityAuditProvider().blockGroupEvents()) {
                        // Log the event
                        webManager.logEvent("added group member to "+groupName, "selectUserName = "+selectUserName);
                    }
                }
                ...
                if (added) {
                    ...
                    errorBuf.append("<br>").append(
                            LocaleUtils.getLocalizedString("group.edit.already_user", Arrays.asList(selectUserName)));
                }
                ...
                Log.warn("Problem adding new user to existing group", e);
                errorBuf.append("<br>").append(
                        LocaleUtils.getLocalizedString("group.edit.inexistent_user", Arrays.asList(selectUserName)));
            }
    	}
    	if (count > 0) {
            response.sendRedirect("dept-edit.jsp?group=" +
        ...
        response.sendRedirect("dept-edit.jsp?group=" + URLEncoder.encode(groupName, "UTF-8") + "&deletesuccess=true");
        return;
    }
...
<meta name="subPageID" content="dept-edit"/>
...
<link rel="stylesheet" type="text/css" href="style/jquery/jquery-ui.css"/>
<link rel="stylesheet" type="text/css" href="style/jquery/ui.multiselect.css"/>
<link rel="stylesheet" type="text/css" href="style/jquery/common.css"/>
<script type="text/javascript" src="scripts/jquery/jquery-1.9.1.js"></script>
<script type="text/javascript" src="scripts/jquery/jquery-ui.js"></script>
<script type="text/javascript" src="scripts/jquery/multiselect/plugins/localisation/jquery.localisation-min.js"></script>
<script type="text/javascript" src="scripts/jquery/multiselect/plugins/scrollTo/jquery.scrollTo-min.js"></script>
<script type="text/javascript" src="scripts/jquery/multiselect/ui.multiselect.js"></script>
<script>
    var jq=$.noConflict();
	jq.localise('ui-multiselect', {/*language: 'en',*/ path: 'scripts/jquery/multiselect/locale/'});
	function selectUsers(){
		...
		jq( "#dialog-message" ).dialog({
		      ...
		      buttons: {
		        Ok: function() {
		        	jq("#selectUsersForm").submit();
		        },
		        cancel: function(){
		        	jq(this).dialog( "close" );
		        }
		      },
		      position: { my: "right center", at: "right-25% top-50%", of: jq(window) } ,
		      width:700,
		      heigth:430,
		    });
	}
...
<p>
	<a href="dept-summary.jsp" class="jive-link-back"><span>&laquo;</span> Back to all groups</a>
</p>
...
<form name="ff" action="dept-edit.jsp">
<input type="hidden" name="group" value="<%= groupName %>"/>
	...
	<div class="jive-contentBox-plain">
        ...
        <div class="jive-contentBox-toolbox">
			<a href="dept-create.jsp?group=<%= URLEncoder.encode(group.getName(), "UTF-8")%>&name=<%= URLEncoder.encode(group.getName(), "UTF-8")%>&description=<%= group.getDescription() != null? URLEncoder.encode(group.getDescription(), "UTF-8") : "" %>" class="jive-link-edit"><fmt:message key="group.edit.edit_details" /></a>
			<a href="dept-delete.jsp?group=<%= URLEncoder.encode(group.getName(), "UTF-8")%>" class="jive-link-delete"><fmt:message key="group.edit.delete" /></a>
		</div>
        <% } %>
	...
	<div id="dialog-message" title="<fmt:message key="selectUser.title" />" style="display: none">
		<form action="dept-edit.jsp" method="post" name="selectUsersForm" id="selectUsersForm">
       	  <input type="hidden" name="group" value="<%= groupName %>">
       	  <input type="hidden" name="add" value="Add"/>
		  <select id="deptUsers" class="multiselect" multiple="multiple" name="deptUsers">
	        <%  for (User u : webManager.getUserManager().getUsers()) {
			%>
				<option value="<%= URLEncoder.encode(u.getUsername(), "UTF-8") %>"
				 ><%= StringUtils.escapeHTMLTags(u.getName()+"--"+u.getUsername()) %></option>
	...
	<div class="jive-contentBox">
        ...
        <form action="dept-edit.jsp" method="post" name="f">
        <input type="hidden" name="group" value="<%= groupName %>">
        <input type="hidden" name="add" value="Add"/>
        <table cellpadding="3" cellspacing="1" border="0" style="margin: 0 0 8px 0;">
            <tr>
                ...
                <td nowrap class="c1" align="left">
                    <input type="text" size="45" name="username" id="username"/>
                    &nbsp;<input type="submit" name="addbutton" value="<fmt:message key="global.add" />" />
                    &nbsp;<input type="button" value="<fmt:message key="global.addUsers" />" onclick="selectUsers()"/>
                </td>
            </tr>
        </table>
        </form>
        ...
        <form action="dept-edit.jsp" method="post" name="main">
        <input type="hidden" name="group" value="<%= groupName %>">
